...
func _ready():
	...
	if $BounceArea:
		...
		print("🐔 Chicken BounceArea connected - Crit Chance Buff Active!")
	else:
		print("ERROR: BounceArea not found on Chicken")
...
func _on_body_entered(body):
	...
	if body.is_in_group("player"):
		print("🐔 Player near chicken - Activating +10% Crit Chance buff!")
		
		# Set the buff active flag
		player_near = true
		GameManager.chicken_buff_active = true
...
func remove_buff_after_delay():
	...
	if not player_near:
		GameManager.chicken_buff_active = false
		print("🐔 Chicken crit buff expired!")
